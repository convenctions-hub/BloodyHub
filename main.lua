...
-- COMBAT: на сколько ниже моба стоять (X из ТЗ, 5..10).
-- Если выставлен _G.KillSnapDepth через слайдер — берём его, это даёт обратную совместимость.
_G.CombatDepth           = _G.CombatDepth           or (_G.KillSnapDepth or 7)
-- QUEST: на сколько отойти вперёд от NPC (в studs), чтобы стоять "лицом к лицу".
_G.QuestForwardDist      = _G.QuestForwardDist      or 4
-- Пороги перетелепорта (чтобы не дёргать каждый кадр).
_G.QuestTeleThreshold    = _G.QuestTeleThreshold    or 3.5
_G.CombatTeleThreshold   = _G.CombatTeleThreshold   or 2.0
-- Debounce между MoveTo* вызовами (секунды).
...
-- Персонаж стоит НОРМАЛЬНО перед NPC, лицом к нему. Никаких Y-offset.
-- Использует NPC CFrame * CFrame.new(0, 0, -forwardDist) — точка перед NPC в его локальных координатах.
...
local function MoveToNPC(npcHRP, opts)
	...
	-- Точка прямо перед NPC (в мировых координатах).
	local npcCF = npcHRP.CFrame
	local targetCF = npcCF * CFrame.new(0, 0, -forwardDist)

	-- Distance check: если мы уже близко к нужной точке, не дёргаем CFrame.
	...
	-- Ставим на ту же высоту, что и NPC (его pivot), НЕ под ним.
	local myPos = targetCF.Position
	-- Смотрим на NPC (взгляд горизонтально).
...
-- Позиция НИЖЕ моба (для битья снизу). Использует target.HumanoidRootPart.CFrame * CFrame.new(0, -X, 0).
local _lastCombatMove = 0
local function makeCombatCFrame(enemyHRP, depth)
	-- Опускаемся по локальной вертикали моба — как просит ТЗ: targetCF * CFrame.new(0, -X, 0).
	local targetCF = enemyHRP.CFrame * CFrame.new(0, -(depth or _G.CombatDepth or 7), 0)
	local myPos = targetCF.Position
	-- Смотрим на моба горизонтально на той же высоте, что и мы.
...
-- ==================== ОБЩИЙ ГОРИЗОНТАЛЬНЫЙ ТЕЛЕПОРТ (для дальних подлётов) ====================
-- Используется только когда до цели далеко и нужно быстро сократить дистанцию,
-- БЕЗ Y-offset. После него вызывается MoveToNPC или MoveToEnemy по ситуации.
...
-- ==================== COMBAT SNAP LOOP (только для боя) ====================
-- Держит игрока НИЖЕ моба каждый кадр, чтобы бить снизу.
local function startCombatSnapLoop(hrp, enemyHRP)
	...
	Log(string.format("COMBAT SNAP: depth=%.1f", depth), Color3.fromRGB(0,255,200))
	...
	local conn = RunService.Heartbeat:Connect(function()
		...
		if enemyHRP and enemyHRP.Parent then
			...
			hrp.CFrame = makeCombatCFrame(enemyHRP, _G.CombatDepth or _G.KillSnapDepth or 7)
...
local function doTalkQuest(targetName, sessionId)
	...
	local ok, err = pcall(function()
		...
		-- 1) Если далеко — сначала горизонтальный подлёт (без Y-offset).
		...
		-- 2) QUEST MODE: встать НОРМАЛЬНО перед NPC.
		...
		for attempt = 1, attemptMax do
			...
			-- Если NPC подвинулся или нас оттолкнуло — корректируем позицию (c threshold/debounce).
			MoveToNPC(npcHRP)
			forceUnlock()
...
local function doKillLoop(targetName, sessionId)
	...
	local ok, err = pcall(function()
		while _G.AutoQuest_Enabled and _G.QuestSessionId == sessionId and not _G.BS_Dead do
			...
			if dist > 15 then
				-- Далеко — быстрый горизонтальный подлёт.
				teleportTo(function()
				...
				-- После подлёта сразу встаём в боевую позицию (ниже моба).
				MoveToEnemy(mobHRP, { force = true })
			else
				-- COMBAT MODE: держим игрока ниже моба и атакуем.
				MoveToEnemy(mobHRP, { force = true })
				local stopSnap = startCombatSnapLoop(hrp, mobHRP)
...
local function doRaidLoop(raidName, raidSid)
	...
	local ok, err = pcall(function()
		...
		-- Подлёт + встать нормально перед рейд-NPC (QUEST MODE).
		local hrp0 = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
...
_G.BloodyHub_API = {
	...
	SetKillSnapDepth = function(v)
		...
		if n then
			_G.KillSnapDepth = n
			_G.CombatDepth   = n  -- слайдер теперь управляет ИСКЛЮЧИТЕЛЬНО боевым offset
		end
	end,
...
Log("BloodyHub v86 LOADED", Color3.fromRGB(0,255,255))
