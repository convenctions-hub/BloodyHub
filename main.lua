...
-- AutoDialog всегда следует за AutoQuest — отдельного флага не нужно
...
-- ==================== DIALOG CLICK (улучшенный) ====================
-- Ищем первую кнопку диалога по нескольким стратегиям
local function clickDialogOption()
	-- Стратегия 1: стандартные Roblox-диалоги через DialogChoice
	local char = LocalPlayer.Character
	if char then
		for _, v in ipairs(workspace:GetDescendants()) do
			if v:IsA("Dialog") and v.InUse then
				local choices = v:GetChildren()
				-- сортируем по ResponseOrder, берём первый
				table.sort(choices, function(a, b)
					if a:IsA("DialogChoice") and b:IsA("DialogChoice") then
				...
				for _, c in ipairs(choices) do
					if c:IsA("DialogChoice") then
						pcall(function()
							-- стреляем через RemoteEvent диалога
						...
						Log("DIALOG CHOICE: " .. tostring(c.ResponseDialog), Color3.fromRGB(0, 255, 200))
						return true
					end
				end
			end
	...
	-- Сначала ищем кнопки с "1." или первые цифровые варианты
	local candidates = {}
	for _, gui in ipairs(playerGui:GetDescendants()) do
		if gui:IsA("TextButton") and gui.Visible and gui.Active then
			local name = gui:GetFullName()
			-- пропускаем наши собственные GUI
			if not name:find("BloodyHub") and not name:find("BSLog") then
				local text = gui.Text or ""
				-- явное "1." или "Yes/Accept/Sure/Ok"
				if text:match("^%s*1[%.]") or
				   text:lower():match("^%s*yes") or
				   text:lower():match("^%s*accept") or
				   text:lower():match("^%s*sure") or
				   text:lower():match("^%s*ok%s*$") then
					table.insert(candidates, { btn = gui, priority = 1 })
				end
			end
		end
	end

	-- Если явных нет — берём просто первую видимую кнопку в диалоговых фреймах
	if #candidates == 0 then
		for _, gui in ipairs(playerGui:GetDescendants()) do
			if gui:IsA("Frame") and gui.Visible then
				local fname = gui:GetFullName()
				if not fname:find("BloodyHub") and not fname:find("BSLog") then
					-- ищем первую TextButton-дочку
					for _, child in ipairs(gui:GetChildren()) do
						if child:IsA("TextButton") and child.Visible then
							table.insert(candidates, { btn = child, priority = 2 })
							break
						end
					end
				end
	...
	-- берём кандидата с наивысшим приоритетом
	table.sort(candidates, function(a, b) return a.priority < b.priority end)
	local btn = candidates[1].btn
	pcall(function()
		for _, c in ipairs(getconnections(btn.MouseButton1Click)) do c:Fire() end
	end)
	pcall(function() firesignal(btn.MouseButton1Click) end)
	pcall(function() firesignal(btn.Activated) end)
	Log("GUI DIALOG BTN: " .. (btn.Text or "?"), Color3.fromRGB(0, 255, 200))
	return true
end
...
local function doTalkQuest(targetName, sessionId)
	...
	local ok, err = pcall(function()
		...
		for _, v in ipairs(workspace:GetDescendants()) do
			if v:IsA("ProximityPrompt") and v.Enabled then
				...
				if part and part:IsA("BasePart")
					and (part.Position - npcHRP.Position).Magnitude < 12 then
					v.HoldDuration = 0
		...
		-- Автодиалог: всегда активен вместе с Master Auto Quest
		-- (AutoDialog_Enabled = AutoQuest_Enabled, задаётся в SetAutoQuest)
		if _G.AutoDialog_Enabled then
			-- Несколько попыток с паузами, чтобы поймать момент открытия диалога
			if not activated then
				Log("NO PROMPT — trying dialog clicks", Color3.fromRGB(255, 150, 0))
			end
			for attempt = 1, 6 do
				task.wait(0.4)
				if _G.QuestSessionId ~= sessionId then break end
				local clicked = clickDialogOption()
				if clicked then
					task.wait(0.3)
					clickDialogOption() -- второй клик на случай цепочки диалогов
					task.wait(0.3)
					clickDialogOption()
					break
				end
...
_G.BloodyHub_API = {
	...
	-- AutoQuest включает/выключает и автодиалог одновременно
	SetAutoQuest = function(v)
		_G.AutoQuest_Enabled  = v and true or false
		_G.AutoDialog_Enabled = _G.AutoQuest_Enabled  -- диалог вшит
		if v then
