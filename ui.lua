--[[ BloodyHub UI v5.0
   Изменения:
     • Combat-таб теперь содержит чистые контролы для нового AttackController:
         - Toggle: Kill Aura (Combat.Start/Stop)
         - Slider: Attack Delay  (sec)
         - Slider: Attack Range  (studs)
         - Button: Fire M1 Once  (ручной тест)
     • Debug-таб переписан:
         - Toggle: Debug Mode (логирует remote'ы по фильтру)
         - Поле: имя remote-фильтра (по умолчанию "M1")
     • Удалены все элементы, связанные с click-detect / VirtualInput.
]]
...
-- ==================== LOGO LOADER ====================
local LOGO_URL  = "https://raw.githubusercontent.com/convenctions-hub/BloodyHub/refs/heads/main/logo.png"
local LOGO_PATH = "BloodyHub/logo.png"

local function loadLogoAsset()
    local ok, asset = pcall(function()
        if not (writefile and isfile and getcustomasset) then return nil end
        if isfolder and not isfolder("BloodyHub") and makefolder then makefolder("BloodyHub") end
...
local LOGO_ASSET = loadLogoAsset()
...
local ScreenGui = new("ScreenGui", {
    Name = "BloodyHub", ResetOnSpawn = false,
...
new("ImageLabel", {
    ...
    ImageColor3 = Color3.fromRGB(130,0,0), ImageTransparency = 0.35,
...
new("Frame", { Parent = TopBar, BorderSizePixel = 0,
    BackgroundColor3 = Color3.fromRGB(140,0,0), Size = UDim2.new(1,0,0,4) }, {
    new("UIGradient", { Transparency = NumberSequence.new({
        NumberSequenceKeypoint.new(0,0.3), NumberSequenceKeypoint.new(0.4,0),
        NumberSequenceKeypoint.new(0.7,0.4), NumberSequenceKeypoint.new(1,0.2),
    }) }),
})

-- ==================== ЛОГОТИП ====================
local LOGO_HEIGHT = 26
local logoContainer = new("Frame", {
    Parent = TopBar, BackgroundTransparency = 1,
    Position = UDim2.fromOffset(14, (40 - LOGO_HEIGHT) / 2),
    Size = UDim2.fromOffset(220, LOGO_HEIGHT),
})
if LOGO_ASSET then
    new("ImageLabel", {
        Parent = logoContainer, BackgroundTransparency = 1, BorderSizePixel = 0,
        Image = LOGO_ASSET, ScaleType = Enum.ScaleType.Fit,
        Size = UDim2.new(0, 220, 1, 0), Position = UDim2.fromOffset(0, 0),
        AnchorPoint = Vector2.new(0, 0), ImageColor3 = Color3.new(1,1,1),
    })
else
    new("TextLabel", {
        Parent = logoContainer, BackgroundTransparency = 1, Size = UDim2.fromScale(1,1),
        Font = FONT, TextSize = 20, TextXAlignment = Enum.TextXAlignment.Left,
        TextYAlignment = Enum.TextYAlignment.Center, RichText = true,
...
winBtn("—", -70, function()
    ...
    tween(Main, 0.25, { Size = minimized and UDim2.fromOffset(WIN_W,40) or origSize })
end)
winBtn("✕", -36, function()
    tween(Main, 0.2, { BackgroundTransparency = 1 })
    task.delay(0.25, function() Main.Visible = false; Main.BackgroundTransparency = 0.05 end)
end)
...
local Bottom = new("Frame", {
    Parent = Main, Size = UDim2.new(1,0,0,30),
    Position = UDim2.new(0,0,1,-30), BackgroundColor3 = THEME.PANEL,
    BorderSizePixel = 0, BackgroundTransparency = 0.2,
}, { corner(8) })
new("Frame", { Parent = Bottom, BorderSizePixel = 0,
    BackgroundColor3 = THEME.PANEL, Size = UDim2.new(1,0,0,12) })
new("Frame", { Parent = Bottom, BorderSizePixel = 0,
...
new("TextLabel", {
    ...
    Text = 'Welcome back, <font color="rgb(255,40,40)">'
        ..(LocalPlayer.DisplayName or LocalPlayer.Name)..'</font>!',
})
new("TextLabel", {
    Parent = Bottom, BackgroundTransparency = 1,
    ...
    TextColor3 = THEME.TEXT_DIM, RichText = true,
    Text = 'Version 5.0.0 <font color="rgb(255,40,40)">|</font> BloodyHub',
})
...
local function makeHeader(parent, title)
    new("TextLabel", {
        ...
        Position = UDim2.fromOffset(14,4), Size = UDim2.new(1,-14,0,26),
        Font = FONT, TextSize = 20, TextColor3 = THEME.TEXT,
...
function BloodyHub:MakeTab(name, iconGlyph)
    local btn = new("TextButton", {
        Parent = Sidebar, Size = UDim2.new(1,-16,0,34),
        BackgroundColor3 = THEME.RED_DEEP, BackgroundTransparency = 1,
        BorderSizePixel = 0, AutoButtonColor = false, Text = "",
    }, { corner(6) })
    local s = stroke(THEME.RED, 1, 1); s.Parent = btn
    local icon = new("TextLabel", {
        Parent = btn, BackgroundTransparency = 1,
        Position = UDim2.fromOffset(10,0), Size = UDim2.fromOffset(20,34),
        Font = FONT, TextSize = 14, TextColor3 = THEME.TEXT_DIM, Text = iconGlyph or "•",
    })
    ...
    local page = new("ScrollingFrame", {
        Parent = Content, Visible = false, Size = UDim2.fromScale(1,1),
    ...
    local function makeSectionRaw(self, title)
        ...
        new("UIListLayout", { Parent = section, Padding = UDim.new(0,6),
            SortOrder = Enum.SortOrder.LayoutOrder })
        ...
        new("TextLabel", {
            ...
            Size = UDim2.new(1,0,0,20), Font = FONT, TextSize = 14,
        ...
        function sec:MakeLabel(text, richText)
            new("TextLabel", {
                ...
                Text = text, LayoutOrder = #section:GetChildren(),
            })
        end

        function sec:MakeKeybind(name, onTriggered)
            local listening, boundKey = false, nil
            local row = new("Frame", { Parent = section, BackgroundTransparency = 1,
                Size = UDim2.new(1,0,0,28), LayoutOrder = #section:GetChildren() })
            new("TextLabel", { Parent=row, BackgroundTransparency=1,
                Size=UDim2.new(1,-110,1,0), Font=FONT_REG, TextSize=13,
                TextColor3=THEME.TEXT, TextXAlignment=Enum.TextXAlignment.Left, Text=name })
            local bindBtn = new("TextButton", {
                Parent=row, AutoButtonColor=false,
                Position=UDim2.new(1,-105,0.5,-11), Size=UDim2.fromOffset(100,22),
                BackgroundColor3=THEME.PANEL, BorderSizePixel=0,
                Font=FONT_REG, TextSize=12, TextColor3=THEME.TEXT_DIM, Text="[ None ]",
            }, { corner(4), stroke(THEME.STROKE,1,0.3) })
            bindBtn.MouseButton1Click:Connect(function()
                if listening then return end
                listening = true; bindBtn.Text = "..."; bindBtn.TextColor3 = THEME.RED
                local conn
        ...
        function sec:MakeToggle(name, default, callback)
            local row = new("Frame", { Parent = section, BackgroundTransparency = 1,
                Size = UDim2.new(1,0,0,28), LayoutOrder = #section:GetChildren() })
            new("TextLabel", { Parent=row, BackgroundTransparency=1,
                Size=UDim2.new(1,-50,1,0), Font=FONT_REG, TextSize=13,
                TextColor3=THEME.TEXT, TextXAlignment=Enum.TextXAlignment.Left, Text=name })
            local tog = new("TextButton", {
                Parent=row, Text="", AutoButtonColor=false,
                Size=UDim2.fromOffset(38,20), Position=UDim2.new(1,-38,0.5,-10),
                BackgroundColor3=default and THEME.RED or THEME.TOGGLE_OFF, BorderSizePixel=0,
            }, { corner(10) })
            local knob = new("Frame", {
                Parent=tog, BorderSizePixel=0, BackgroundColor3=Color3.new(1,1,1),
                Size=UDim2.fromOffset(14,14),
                Position=UDim2.new(default and 1 or 0, default and -17 or 3, 0.5,-7),
            }, { corner(7) })
            local state = default and true or false
            local function apply()
                tween(tog, 0.18, { BackgroundColor3 = state and THEME.RED or THEME.TOGGLE_OFF })
                tween(knob, 0.18, { Position = UDim2.new(state and 1 or 0, state and -17 or 3, 0.5,-7) })
                if callback then pcall(callback, state) end
            end
            tog.MouseButton1Click:Connect(function() state = not state; apply() end)
            return {
        ...
        function sec:MakeSlider(name, min, max, default, decimals, callback)
            ...
            local row = new("Frame", { Parent=section, BackgroundTransparency=1,
                Size=UDim2.new(1,0,0,40), LayoutOrder=#section:GetChildren() })
            new("TextLabel", { Parent=row, BackgroundTransparency=1, Size=UDim2.new(1,-60,0,18),
                Font=FONT_REG, TextSize=13, TextColor3=THEME.TEXT,
                TextXAlignment=Enum.TextXAlignment.Left, Text=name })
            local val = new("TextLabel", { Parent=row, BackgroundTransparency=1,
                Position=UDim2.new(1,-60,0,0), Size=UDim2.fromOffset(60,18),
                Font=FONT, TextSize=13, TextColor3=THEME.RED,
                TextXAlignment=Enum.TextXAlignment.Right, Text=tostring(default) })
            local bar = new("Frame", { Parent=row, BorderSizePixel=0,
                BackgroundColor3=THEME.TOGGLE_OFF, Position=UDim2.fromOffset(0,24),
                Size=UDim2.new(1,0,0,6) }, { corner(3) })
            local fill = new("Frame", { Parent=bar, BorderSizePixel=0,
                BackgroundColor3=THEME.RED,
                Size=UDim2.new((default-min)/(max-min),0,1,0) }, { corner(3) })
            local handle = new("Frame", { Parent=bar, BorderSizePixel=0,
                BackgroundColor3=Color3.new(1,1,1), Size=UDim2.fromOffset(12,12),
                AnchorPoint=Vector2.new(0.5,0.5),
                Position=UDim2.new((default-min)/(max-min),0,0.5,0) }, { corner(6) })
            ...
            bar.InputBegan:Connect(function(i)
                if i.UserInputType==Enum.UserInputType.MouseButton1
                   or i.UserInputType==Enum.UserInputType.Touch then
                    sliding=true; setFromX(i.Position.X) end
            end)
            UserInputSvc.InputChanged:Connect(function(i)
        ...
        function sec:MakeTextInput(name, default, callback)
            local row = new("Frame", { Parent=section, BackgroundTransparency=1,
                Size=UDim2.new(1,0,0,28), LayoutOrder=#section:GetChildren() })
            new("TextLabel", { Parent=row, BackgroundTransparency=1,
                Size=UDim2.new(0.5,-4,1,0), Font=FONT_REG, TextSize=13,
                TextColor3=THEME.TEXT, TextXAlignment=Enum.TextXAlignment.Left,
                Text=name })
            local box = new("TextBox", {
                Parent=row, AutoButtonColor=false,
                Position=UDim2.new(0.5,4,0.5,-11), Size=UDim2.new(0.5,-4,0,22),
                BackgroundColor3=THEME.PANEL, BorderSizePixel=0,
                Font=FONT_REG, TextSize=12, TextColor3=THEME.TEXT,
                PlaceholderColor3=THEME.TEXT_DIM, ClearTextOnFocus=false,
        ...
        function sec:MakeRaidDropdown(options, callback)
            ...
            local container = new("Frame", { Parent = section, BackgroundTransparency = 1,
                Size = UDim2.new(1,0,0,32), LayoutOrder = #section:GetChildren(),
            ...
            local listFrame = new("Frame", {
                ...
                Size = UDim2.new(1,0,0,#options*26+8),
                BackgroundColor3 = THEME.PANEL, BorderSizePixel = 0,
                ZIndex = 20, ClipsDescendants = false,
            }, { corner(5), stroke(THEME.RED,1,0.3) })
            new("UIListLayout", { Parent=listFrame, Padding=UDim.new(0,2) })
            new("UIPadding", { Parent=listFrame,
                PaddingTop=UDim.new(0,4), PaddingBottom=UDim.new(0,4),
                PaddingLeft=UDim.new(0,4), PaddingRight=UDim.new(0,4) })
            local listOpen, current = false, nil
            local function closeList()
                listOpen = false
                tween(listFrame, 0.15, { BackgroundTransparency = 1 })
                task.delay(0.15, function()
                    listFrame.Visible = false; listFrame.BackgroundTransparency = 0
                end)
            end
            local function openList()
                listOpen = true
                listFrame.Visible = true; listFrame.BackgroundTransparency = 1
                tween(listFrame, 0.15, { BackgroundTransparency = 0 })
            end
            for _, opt in ipairs(options) do
                local optBtn = new("TextButton", {
                    Parent = listFrame, AutoButtonColor = false,
                    Size = UDim2.new(1,0,0,24),
                    BackgroundColor3 = THEME.PANEL_ALT, BackgroundTransparency = 0.4,
                    BorderSizePixel = 0,
                    Font = FONT_REG, TextSize = 12, TextColor3 = THEME.TEXT,
                    TextXAlignment = Enum.TextXAlignment.Left,
                    Text = "  "..opt, ZIndex = 21,
                ...
                optBtn.MouseEnter:Connect(function()
                    tween(optBtn, 0.1, { BackgroundColor3=THEME.RED_DEEP, BackgroundTransparency=0 })
                    optBtn.TextColor3 = THEME.RED
                end)
                optBtn.MouseLeave:Connect(function()
                    tween(optBtn, 0.1, { BackgroundColor3=THEME.PANEL_ALT, BackgroundTransparency=0.4 })
        ...
        function sec:MakeButton(name, callback)
            local b = new("TextButton", {
                ...
                Size=UDim2.new(1,0,0,28), BackgroundColor3=THEME.RED_DEEP,
                BorderSizePixel=0, Font=FONT, TextSize=13,
                TextColor3=THEME.TEXT, Text=name, LayoutOrder=#section:GetChildren(),
    ...
    function tab:MakeSection(title)
        if not pendingRow or (pendingRow:GetAttribute("count") or 0) >= 2 then
            pendingRow = new("Frame", { Parent=body, BackgroundTransparency=1,
...
local TabDebug    = BloodyHub:MakeTab("Debug",     "⚙")
...
-- ==================== COMBAT TAB ====================
-- Чистый M1: всё через RemoteEvent. Никаких кликов мыши.
local secCombat = TabCombat:MakeSection("M1 / Kill Aura")
secCombat:MakeLabel("Атака идёт через RemoteEvent\nWorkspace.Live.<Char>.client_character_controller.M1\nFireServer(true, false). Click-симуляция отключена.", false)
secCombat:MakeToggle("Kill Aura (auto M1)", false, function(v)
    safeCall(API.SetCombat, v)
end)
secCombat:MakeSlider("Attack Delay (sec)", 0.05, 1.5, 0.30, 2, function(v)
    safeCall(API.SetAttackDelay, v)
end)
secCombat:MakeSlider("Attack Range (studs)", 4, 60, 18, 0, function(v)
    safeCall(API.SetAttackRange, v)
end)
secCombat:MakeButton("Fire M1 Once (test)", function()
    safeCall(API.FireM1Once)
end)

local secCombatAdv = TabCombat:MakeSection("Positioning")
secCombatAdv:MakeSlider("Combat Y Offset", 1, 30, 6, 1, function(v)
    safeCall(API.SetCombatYOffset, v)
end)
secCombatAdv:MakeSlider("Move Tick Rate", 0.05, 0.5, 0.12, 2, function(v)
    safeCall(API.SetMoveTickRate, v)
end)
secCombatAdv:MakeSlider("Move Lerp Alpha", 0.05, 1, 0.35, 2, function(v)
    safeCall(API.SetMoveLerpAlpha, v)
end)

-- ==================== AUTO FARM ====================
local secFarm = TabAutoFarm:MakeSection("Quests")
secFarm:MakeToggle("Master Auto Quest", false, function(v) safeCall(API.SetAutoQuest, v) end)

local secRaid = TabAutoFarm:MakeSection("Auto Raid")
secRaid:MakeToggle("Auto Raid On", false, function(v) safeCall(API.SetAutoRaid, v) end)
local RAID_OPTIONS = {
    "Muhammad Avdol Raid","Jotaro Kujo Raid","Kira Yoshikage Raid","Dio Brando Raid",
    "Prison Escape Raid","Death 13 Raid","Twoh Raid",
}
secRaid:MakeRaidDropdown(RAID_OPTIONS, function(s) safeCall(API.SetRaidSelected, s) end)
secRaid:MakeToggle("Auto Retry",  false, function(v) safeCall(API.SetAutoRaidRetry, v) end)
secRaid:MakeToggle("Auto Return", false, function(v) safeCall(API.SetAutoRaidReturn, v) end)

-- ==================== DEBUG TAB (clean) ====================
local secDbg = TabDebug:MakeSection("Remote Logger")
secDbg:MakeLabel("Логирует только RemoteEvent / RemoteFunction, у которых имя содержит фильтр (без учёта регистра). По умолчанию — \"M1\". Используй для проверки: должен ли M1 действительно срабатывать.", false)
secDbg:MakeToggle("Debug Mode (filtered logger)", false, function(v)
    safeCall(API.SetDebug, v)
end)
secDbg:MakeTextInput("Filter (substring)", "M1", function(text)
    safeCall(API.SetDebugFilter, text)
end)

-- ==================== VISUALS ====================
local secVis = TabVisuals:MakeSection("ESP")
secVis:MakeToggle("Player ESP", false, function(v) safeCall(API.SetESP, v) end)

-- ==================== SESSION ====================
local secSess = TabSession:MakeSection("Session")
secSess:MakeButton("⛔ Destroy Session", function() safeCall(API.DestroySession) end)

-- ==================== SETTINGS ====================
local secSets = TabSettings:MakeSection("Interface")
secSets:MakeKeybind("Show/Hide UI", function()
...
secCred:MakeLabel('BloodyHub v5.0.0\nClean combat (M1 RemoteEvent only).\nNo mouse-click simulation.\n© 2026\n\nCreator: bloodytears', false)
...
_G.BloodyHub_UI = {
    ...
    Show = function()
        Main.Visible = true; Main.BackgroundTransparency = 1
        Main.Size = UDim2.fromOffset(WIN_W * 0.8, WIN_H * 0.8)
        tween(Main, 0.35, { Size=origSize, BackgroundTransparency=0.05 }, Enum.EasingStyle.Back)
    end,
