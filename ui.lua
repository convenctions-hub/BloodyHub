--[[ BloodyHub UI v3
- Заголовок "BloodyHub" заменён на ImageLabel с логотипом
- Логотип скачивается через HttpGet и кешируется в файл, ImageLabel.Image
  получает content://-URL через getcustomasset
- Добавлены тогглы для RemoteSpy / InputDebug и кнопка StartAttackDetect
]]
...
-- ==================== LOGO LOADER ====================
-- Скачиваем logo.png в локальный файл и получаем content:// через getcustomasset.
...
-- Высота тулбара = 40. Для аккуратного вида берём логотип высотой ~26px,
-- ширина авто-подгоняется через UIAspectRatioConstraint.
-- Если getcustomasset недоступен — fallback на текст, чтобы UI не сломался.
...
local logoContainer = new("Frame", {
	...
	Size = UDim2.fromOffset(220, LOGO_HEIGHT), -- широкая «обёртка»; реальная картинка обрежется
})
...
if LOGO_ASSET then
	local logo = new("ImageLabel", {
		...
		BorderSizePixel = 0,
		Image = LOGO_ASSET,
		ScaleType = Enum.ScaleType.Fit, -- сохраняем aspect ratio внутри контейнера
		Size = UDim2.new(0, 220, 1, 0), -- высота = высоте контейнера, ширина с запасом
		Position = UDim2.fromOffset(0, 0),
		AnchorPoint = Vector2.new(0, 0),
		ImageColor3 = Color3.new(1,1,1),
	})
	-- ScaleType.Fit уже хранит aspect ratio; UIAspectRatioConstraint не нужен,
	-- но добавим лёгкую тень-обводку, чтобы лого читался на любом фоне.
	new("UIStroke", {
		Parent = logo,
		Thickness = 0,
		Transparency = 1,
	...
	-- fallback — старый текст
	new("TextLabel", {
		Parent = logoContainer, BackgroundTransparency = 1,
...
local function winBtn(txt, xoff, cb)
	local b = new("TextButton", {
		Parent = TopBar, BackgroundTransparency = 1,
		Text = txt, Font = FONT, TextSize = 22, TextColor3 = THEME.TEXT_DIM,
...
new("TextLabel", {
	...
	TextColor3 = THEME.TEXT_DIM, RichText = true,
	Text = 'Version 2.0.0 <font color="rgb(255,40,40)">|</font> BloodyHub',
})
...
secFarm:MakeSlider("Kill Snap Depth", 1, 30, 5, 1, function(v) safeCall(API.SetKillSnapDepth, v) end)
...
secRaid:MakeRaidDropdown(RAID_OPTIONS, function(selected) safeCall(API.SetRaidSelected, selected) end)
...
-- ==================== DEBUG TAB ====================
local secDbg = TabDebug:MakeSection("Remote / Attack Debug")
secDbg:MakeLabel("1) Включи RemoteSpy.\n2) Нажми «Detect Attack Remote» и сделай ОДИН M1 по мобу.\n3) Скрипт запомнит remote и будет дёргать его сам.", false)
secDbg:MakeToggle("Remote Spy (log all FireServer)", false, function(v)
	safeCall(API.ToggleRemoteSpy, v)
end)
secDbg:MakeToggle("Input Debug (log mouse clicks)", false, function(v)
	safeCall(API.ToggleInputDebug, v)
end)
secDbg:MakeButton("Detect Attack Remote", function() safeCall(API.StartAttackDetect) end)
secDbg:MakeButton("Clear Captured Remote", function() safeCall(API.ClearAttackRemote) end)
secDbg:MakeButton("Show Captured Remote", function()
	if API.Log and API.GetAttackRemoteInfo then
		API.Log("Attack remote: "..tostring(API.GetAttackRemoteInfo()), Color3.fromRGB(0,255,200))
	end
end)
...
secCred:MakeLabel('BloodyHub v2.0.0\nCustom UI, no Rayfield.\n© 2026\n\nCreator: bloodytears', false)

-- ==================== EXPOSE + DISMISS LOADING ====================
_G.BloodyHub_UI = {
