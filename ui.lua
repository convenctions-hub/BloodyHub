    ...
    local function makeSectionRaw(self, title, col)
        ...
        function sec:MakeSlider(name, min, max, default, decimals, callback)
            ...
            return {
                ...
                Set = function(_, v) current = v; setFromX(bar.AbsolutePosition.X + bar.AbsoluteSize.X * ((v - min) / (max - min))) end,
            }
        end
...
-- AUTO FARM (колбэки идут в main.lua через API)
...
-- SETTINGS — кейбинд на показ окна
