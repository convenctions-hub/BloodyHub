...
new("TextLabel", {
    ...
    Position = UDim2.fromOffset(16,0), Size = UDim2.new(0,200,1,0),
...
secFarm:MakeSlider("Kill Snap Depth", 1, 30, 5, 1, function(v)
    safeCall(API.SetKillSnapDepth, v)
end)

-- ==================== AUTO FARM — колонка 2: Auto Raid ====================
local secRaid = TabAutoFarm:MakeSection("Auto Raid")

secRaid:MakeToggle("Auto Raid On", false, function(v)
    safeCall(API.SetAutoRaid, v)
end)
...
secRaid:MakeRaidDropdown(RAID_OPTIONS, function(selected)
    safeCall(API.SetRaidSelected, selected)
end)

secRaid:MakeToggle("Auto Retry", false, function(v)
    safeCall(API.SetAutoRaidRetry, v)
end)

secRaid:MakeToggle("Auto Return", false, function(v)
    safeCall(API.SetAutoRaidReturn, v)
end)
